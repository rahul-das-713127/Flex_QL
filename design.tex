...
\begin{itemize}[leftmargin=*]
\item If \texttt{FLEXQL\_PRESERVE\_DATA=1} is \emph{not} set, the server wipes the data directory on start.
\item If \texttt{FLEXQL\_PRESERVE\_DATA=1} \emph{is} set, the server preserves existing on-disk data (including WAL/catalog).
\end{itemize}

This provides an explicit toggle between ``fresh DB per start'' and ``persistent DB across restarts''.
...
\begin{itemize}[leftmargin=*]
\item An in-memory cache stores results keyed by the SQL string.
\item Cache is an LRU-like structure with bounded capacity.
\item Cache entries are used only for safe/eligible SELECTs (small results).
\end{itemize}
...
\begin{itemize}[leftmargin=*]
\item Inserts set a global dirty flag (atomic) or directly clear cache depending on the code path.
\end{itemize}
...
\item \textbf{Per-table spinlock} is used in the hot insert path to reduce overhead under heavy insert loads.
\end{itemize}
...
\begin{itemize}[leftmargin=*]
\item SELECT execution may require flushing pending write arenas before reads for visibility.
...
Variable & Meaning \\
\midrule
\texttt{FLEXQL\_PRESERVE\_DATA=1} & Preserve \texttt{data/} across server restarts (otherwise wiped on start). \\
...
\begin{itemize}[leftmargin=*]
\item Expired rows are filtered at read time; there is no vacuum/compaction.
\item WAL replay is SQL-replay based; it re-executes statements rather than replaying page-level changes.
\item Indexing is currently focused on a probable INT primary key column.
